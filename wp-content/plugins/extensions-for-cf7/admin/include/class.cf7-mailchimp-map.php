    ...
    */
    ...
    private function cf7_form_tag_html( $formfield, $form_tag_list, $extcf7_mcmp, $filter_txt, $merge_tag ){
    	...
    	?>
    		...
    		<select class="extcf7-mailchimp-select" id="extcf7-mailchimp-<?php echo $formtype;?>" name="<?php echo $formname; ?>" style="width: 95%;">
    			<option <?php  if ( $custom_value == ' ' ) { echo 'selected="selected"'; } ?> disabled>
    				 <?php echo (($custom_value=='email') ? 'Required by MailChimp': 'Choose'); ?>
    			</option>
    		    <?php 
    			foreach ( $tag_list as $list ) {
	              $field = '['. trim( $list['name'] ) . ']' ;
	              ?>
	                <option value="<?php echo $field ?>" <?php if (  trim( $custom_value ) == $field ){ echo 'selected="selected"'; } ?> >
	                  <?php echo '['.$list['name'].']' ?>
	                </option>
	              <?php
		        }
